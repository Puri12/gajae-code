...
The consensus roles are filled by the read-only OmO agents `metis` (Planner), `oracle` (Architect), and `momus` (Critic). They return their plan, review, and evaluation markdown to the leader, which persists each stage via `gjc ralplan --write --artifact "..."` with the markdown passed directly (never a file path), so a verdict command cannot persist arbitrary file contents.

After a role agent persists a stage artifact, its model-facing response to the caller SHOULD be receipt-only: return the `gjc ralplan --write --json` receipt (`run_id`, `path`, `stage`, `stage_n`, `sha256`, `created_at`) plus the minimal verdict/status fields the caller needs for routing, and do **not** paste the full persisted markdown back into the parent conversation. Downstream reviewers should receive the artifact path/receipt and read the persisted file themselves when they actually need the body. This preserves the audit trail while preventing the metis/oracle/momus verdict bodies from being duplicated into the main-agent context.

RECEIPT-ONLY guideline: the leader persists each consensus stage produced by `metis`, `oracle`, and `momus` via `gjc ralplan --write` and keeps ONLY the receipt fields (`run_id`, `path`, `sha256`) plus verdict/status routing fields; include `stage` and `stage_n` when available, and never return the full persisted body.

This skill runs GJC planning in consensus mode for the provided arguments.

The consensus workflow:
1. **Planner** (delegate to `metis`) creates the initial plan and a compact **RALPLAN-DR summary** before review. Launch `metis` ONCE per run as a detached, resumable subagent (await it before the Architect) and record its returned subagent id as the run's persisted Planner id; persist the stage with `gjc ralplan --write --stage planner --stage_n 1 --artifact "..." --planner-id <id> --planner-resumable <true|false>` (see **Persisted Planner** below):
...
3. **Architect** (delegate to `oracle`) reviews for architectural soundness and must provide the strongest steelman antithesis, at least one real tradeoff tension, and (when possible) synthesis — **await completion before step 4**. In deliberate mode, `oracle` should explicitly flag principle violations.
   - Persist the `oracle` review with `gjc ralplan --write --stage architect --stage_n <N> --artifact "..." --json`, then keep the receipt/path plus compact verdict/status (`CLEAR`/`WATCH`/`BLOCK`, `APPROVE`/`COMMENT`/`REQUEST CHANGES`) instead of the full review body.
4. **Critic** (delegate to `momus`) evaluates against quality criteria — run only after step 3 completes. `momus` must enforce principle-option consistency, fair alternatives, risk mitigation clarity, testable acceptance criteria, and concrete verification steps. In deliberate mode, `momus` must reject missing/weak pre-mortem or expanded test plan.
   - Persist the `momus` evaluation with `gjc ralplan --write --stage critic --stage_n <N> --artifact "..." --json`, then keep the receipt/path plus compact verdict/status (`OKAY`/`ITERATE`/`REJECT`) instead of the full evaluation body.
